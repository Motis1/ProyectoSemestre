...
public class SportCenter {
    ...
    private boolean horarioInvalido(Date inicio, Date fin){
        ...
        int horaFin = finNuevo.get(java.util.Calendar.HOUR_OF_DAY);
    ...
    private Cancha buscarCanchaAlternativa(String datosCancha,Date inicio, Date fin){
        Cancha canchaMantenimiento = buscarCancha(datosCancha);
        for (Cancha c : myCanchas) {
            
            if(!c.getCodigo().equals(canchaMantenimiento.getCodigo()) && c.getTipo().equals(canchaMantenimiento.getTipo())){
                String canchaB = c.getCodigo() + " - " + c.getTipo();
                if(!validarCanchas(canchaB, inicio, fin)){
                    return c;
                }
            }
        } 
    ...
    public String cambiarMantenimiento(String datosCancha){
        String reporte = "REPORTE DE MANTENIMIENTO\n============================\n\n";
        Cancha canchaDañada = buscarCancha(datosCancha);
        canchaDañada.setEstado("MANTENIMIENTO");
        
        int reprogramados = 0;
        int canceladas = 0;
        float totalDevuelto = 0;
        
        for (Reserva r : myReservas) {
            if(r.getEstado().equals("ACTIVA")&& r.getMyCancha().equals(canchaDañada)){
                Cancha PlanB = buscarCanchaAlternativa(datosCancha, r.getFechaInicio(), r.getFechaFin());
                ...
                if(PlanB!= null){
                    ...
                    float devolucion = r.getValorPagar()* 1.9F;
                    ganancias -= devolucion;
                    perdidas += devolucion;
                    canceladas++;
                }
            }
        ...
        return reporte + 
                "Cancha inhabilitada: " + canchaDañada.getCodigo() + 
                "\nReprogramadas: " + reprogramados + 
                "\nCanceladas: " + canceladas;
    }
    
    public String reporteGerencial(){
        String reporteActivas = "<<<<<<<<<<<<RESERVAS ACTIVAS>>>>>>>>>>>>\n\n";
        String reporteCanceladas = "<<<<<<<<<<<<RESERVAS CANCELADAS>>>>>>>>>>>>\n\n";
        String recaudos = "\nTotal recaudado por reservas activas: " + ganancias +
                          "\nTotal devolucion por cancelaciones: " + perdidas;
        
        for (Reserva r : myReservas) {
            if(r.getEstado().equalsIgnoreCase("ACTIVA")){
                reporteActivas += r.toString() + "\n-----------------------------------------\n";
            }
            if(r.getEstado().equalsIgnoreCase("CANCELADA")){
                reporteCanceladas += r.toString() + "\n-----------------------------------------\n";
            }
        } 
        return reporteActivas + reporteCanceladas + recaudos;
    }
}
